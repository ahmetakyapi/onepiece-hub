...
import { Clock, Star, BookOpen, Compass, Anchor, Flame, Swords, Crown, Globe, Sparkles, Search, X } from 'lucide-react'
...
const TIMELINE: TimelineEvent[] = [
  ...
  { year: 'Yıl 1', title: 'Arlong Park Yıkıldı', description: 'Luffy, Arlong\'u yenip Nami\'yi 8 yıllık esaretten kurtardı. "Bana yardım et!" repliği One Piece\'in en duygusal anlarından biri.', category: 'east-blue', importance: 3, relatedArc: 'Arlong Park', relatedArcSlug: 'arlong-park' },
  ...
  return (
      <main className="relative min-h-screen pt-24">
        <div className="mx-auto max-w-5xl px-6">
          ...
          {/* Sekansın ne yaptığını anlatır. Sekans desktop-only olduğu için
              banner da aynı breakpoint'te. */}
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: '-40px' }}
            transition={{ duration: 0.6, ease: EASE }}
            className="relative mx-auto mb-6 hidden max-w-xl items-center gap-3 rounded-2xl border border-gold/20 bg-ocean-surface/30 px-5 py-4 backdrop-blur-sm md:flex"
          >
            ...
            <p className="flex-1 text-sm leading-relaxed text-pirate-text">
              Aşağıdaki sekans altı dönemi sırayla açar: Antik Tarih&apos;ten
              New World&apos;e 900 yıl.
            </p>
          </motion.div>
        </div>
