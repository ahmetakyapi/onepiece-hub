    ...
    <div className="relative h-full w-full overflow-hidden bg-[#050b14]">
      ...
      {showGeometryGuides && zoomed && loadState === 'ready' && (
        <div className="pointer-events-none absolute inset-0 z-20" aria-hidden="true">
          ...
          <span className="eyebrow absolute left-2 top-2 rounded bg-black/85 px-2 py-1 text-gold">
      ...
      {loadState === 'failed' && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-4 px-6 text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            ...
            <button onClick={onUseFullEmbed} /* `.btn-ghost` KULLANILAMAZ: sahne iki temada da koyu kalıyor ama
   ghost butonu rengini temadan alıyor — light'ta metin lacivere,
   hover'da deniz mavisine dönüp koyu sahnede ~1.5:1'e düşüyordu.
   Bu blok bilerek sabit "beyaz taraf" tonlarında. */
className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/[0.06] px-4 py-2 text-xs font-semibold text-white backdrop-blur-md transition-colors hover:border-gold/40 hover:bg-gold/15 hover:text-gold">
            ...
            <a
              ...
              /* `.btn-ghost` KULLANILAMAZ: sahne iki temada da koyu kalıyor ama
   ghost butonu rengini temadan alıyor — light'ta metin lacivere,
   hover'da deniz mavisine dönüp koyu sahnede ~1.5:1'e düşüyordu.
   Bu blok bilerek sabit "beyaz taraf" tonlarında. */
className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/[0.06] px-4 py-2 text-xs font-semibold text-white backdrop-blur-md transition-colors hover:border-gold/40 hover:bg-gold/15 hover:text-gold"
